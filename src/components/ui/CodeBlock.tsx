...
function CopyButton({ getText, className = '' }: { getText: () => string; className?: string }): JSX.Element {
  ...
  return (
    <button
      ...
      className={cx(
        'inline-flex items-center gap-1 rounded-lg border border-zinc-700 bg-zinc-900/80 px-2 py-1 text-xs text-zinc-400 transition-colors hover:border-zinc-500 hover:text-zinc-200 hover:shadow-sm hover:shadow-accent-500/10',
        className,
      )}
...
export function CodeBlock({ code, lang = 'bash', title, className = '' }: CodeBlockProps): JSX.Element {
  return (
    <figure
      className={cx(
        'overflow-hidden rounded-xl border border-zinc-200 bg-zinc-50 shadow-sm transition-shadow dark:border-zinc-800 dark:bg-zinc-950',
        'hover:shadow-md hover:shadow-accent-500/10 dark:hover:shadow-accent-500/10',
        className,
      )}
    >
...
export function ICode({ children }: { children: string }): JSX.Element {
  return (
    <code className="rounded-md border border-zinc-300 bg-zinc-100 px-1.5 py-0.5 font-mono text-[0.85em] text-accent-700 shadow-sm shadow-accent-500/0 transition-shadow hover:shadow-accent-500/10 dark:border-zinc-700 dark:bg-zinc-800 dark:text-accent-300">
      {children}
    </code>
  )
}
